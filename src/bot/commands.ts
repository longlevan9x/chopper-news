...
/**
 * Chọn Model cụ thể cho từng Provider
 */
export async function handleModelCommand(ctx: Context, _env: AppEnv): Promise<void> {
  const kb = Markup.inlineKeyboard([
    [Markup.button.callback('🚀 Cấu hình GROQ', 'model_select_groq')],
    [Markup.button.callback('🐦 Cấu hình xAI', 'model_select_xai')],
    [Markup.button.callback('☁️ Cấu hình CLOUDFLARE', 'model_select_cloudflare')]
  ]);

  await ctx.reply('⚙️ <b>BẠN MUỐN CẤU HÌNH MODEL CHO BÊN NÀO?</b>\n<i>Chọn một nhà cung cấp bên dưới:</i>', {
    parse_mode: 'HTML',
    ...kb
  });
}
